...
def score(
    ...
    if num_layers is None:
        if '/' in str(model_type): 
            real_model_type = str(model_type).split('/')[-2] + '/' + str(model_type).split('/')[-1]
            num_layers = model2layers[real_model_type]
        else:
            num_layers = model2layers[model_type]

    tokenizer = get_tokenizer(str(model_type), use_fast_tokenizer)
    model = get_model(str(model_type), num_layers, all_layers)
    if device is None:
        ...
        if baseline_path is None:
            if '/' in model_type: 
                baseline_path = os.path.join(
                    ...
                    f"rescale_baseline/{lang}/{real_model_type}.tsv",
                )
            else:
                baseline_path = os.path.join(
                    os.path.dirname(__file__),
