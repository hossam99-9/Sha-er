...
class BERTScorer:
    ...
    def __init__(
        ...
        if num_layers is None:
            if '/' in model_type: 
                real_model_type = str(model_type).split('/')[-2] + '/' + str(model_type).split('/')[-1]
                self._num_layers = model2layers[real_model_type]
            else:
        ...
        self._tokenizer = get_tokenizer(str(self.model_type), self._use_fast_tokenizer)
        self._model = get_model(str(self.model_type), self.num_layers, self.all_layers)
        ...
        if self.baseline_path is None:
            if '/' in model_type: 
                ...
                f"rescale_baseline/{self.lang}/{real_model_type}.tsv",
            )
            else:
                self.baseline_path = os.path.join(
                os.path.dirname(__file__),
