...
class RhetoricalElementsAgent(BaseAgent):
    ...
    def _prepare_rag_prompt(self):
        self.rag_prompt = ChatPromptTemplate.from_messages([
            ("user", "{الشرح}"),
            ("system", " انت خبير في علم البلاغة في اللغة العربية و معك شرح مختلف ﻷقسام البلاغة و الانواع المختلفة سوف أرفق لك بيت شعري و أنت تستخرج مواطن البلاغة والجمال المختلفة"),
            ("user", "{البيت}"),
        ])
